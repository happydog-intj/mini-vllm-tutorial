...
朴素推理（step05）
  └→ 问题：每步重算全部历史
      └→ KV Cache（step08）：缓存历史 K/V
          └→ 新问题：多请求时 GPU 槽位空转
              └→ Continuous Batching（step09）：动态调度请求
                  └→ 新问题：长 Prefill 阻塞 Decode
                      └→ Chunked Prefill（step10）：切片执行
                          └→ 新问题：KV Cache 显存碎片、OOM
                              ├→ Preemption（step11）：优雅降级
                              └→ PagedAttention（step12）：分页管理
                                  └→ 新问题：注意力计算带宽成瓶颈
                                      └→ FlashAttention（step16）：SRAM 分块
                                          └→ 工程化：多卡、服务化...
```
...
**问题：** step05 每步都重算全部历史 token 的 K/V，大量重复计算。
...
**问题：** 前缀缓存（step13）在 PagedAttention 框架下如何高效实现？
...
```
「重复计算」这个问题：
  step07  KV Cache         → 缓存已算的 K/V，避免逐 token 重算
  step13   Prefix Caching   → 缓存共享 prefix 的 K/V，避免逐请求重算
  step16   FlashAttention   → 避免 attention score 矩阵写回显存再读回

「内存浪费」这个问题：
  step08  Padding          → 引入问题：补 PAD 浪费计算
  step09   Cont. Batching   → 解决：短请求完成即释放槽位
  step12   PagedAttention   → 解决：不预分配 max_len，按需分配 Block
  step16   varlen 接口      → 解决：拼接输入取代 padding，零浪费

「调度延迟」这个问题：
  step09   Scheduler        → 毫秒级：请求级调度，减少 GPU 空转
  step10  Chunked Prefill  → 毫秒级：防止长 prefill 独占 GPU
  step17   CUDA Graph       → 微秒级：跳过 Python/driver 调度开销
```
