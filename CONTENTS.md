...
朴素推理（step01）
  └→ 问题：每步重算全部历史
      └→ KV Cache（step03）：缓存历史 K/V
          └→ 新问题：多请求时 GPU 槽位空转
              └→ Continuous Batching（step04）：动态调度请求
                  └→ 新问题：长 Prefill 阻塞 Decode
                      └→ Chunked Prefill（step05a）：切片执行
                          └→ 新问题：KV Cache 显存碎片、OOM
                              ├→ Preemption（step05b）：优雅降级
                              └→ PagedAttention（step06）：分页管理
                                  └→ 新问题：注意力计算带宽成瓶颈
                                      └→ FlashAttention（step10）：SRAM 分块
                                          └→ 工程化：多卡、服务化...
```
...
### step00d — Transformer Decoder 层：组装完整计算单元
...
**问题：** step01 每步都重算全部历史 token 的 K/V，大量重复计算。
...
### step04 — Continuous Batching 调度器
...
### step05a — Chunked Prefill：切片长 Prompt
...
### step07 — Prefix Caching：相同前缀只算一次
...
### step08 — Paged Prefix Cache（待实现）

**问题：** 前缀缓存（step07）在 PagedAttention 框架下如何高效实现？

**方案：** 待实现。

---

### step09 — 真实模型：加载 Qwen3-0.6B
...
### step10 — FlashAttention：SRAM-aware 注意力计算
...
### step12 — Tensor Parallelism：多卡分布式推理
...
### step14 — HTTP Serve：OpenAI 兼容推理服务
...
```
「重复计算」这个问题：
  step03a  KV Cache         → 缓存已算的 K/V，避免逐 token 重算
  step07   Prefix Caching   → 缓存共享 prefix 的 K/V，避免逐请求重算
  step10   FlashAttention   → 避免 attention score 矩阵写回显存再读回

「内存浪费」这个问题：
  step03b  Padding          → 引入问题：补 PAD 浪费计算
  step04   Cont. Batching   → 解决：短请求完成即释放槽位
  step06   PagedAttention   → 解决：不预分配 max_len，按需分配 Block
  step10   varlen 接口      → 解决：拼接输入取代 padding，零浪费

「调度延迟」这个问题：
  step04   Scheduler        → 毫秒级：请求级调度，减少 GPU 空转
  step05a  Chunked Prefill  → 毫秒级：防止长 prefill 独占 GPU
  step11   CUDA Graph       → 微秒级：跳过 Python/driver 调度开销
```
