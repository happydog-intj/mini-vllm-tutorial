...
> 整数 42 和 43 只差 1，但对应的词可能毫无关系。Embedding 让每个 token 拥有一个可学习的向量身份。

## 这一章做什么？

实现一个 Embedding 查找表，把 token id 映射为连续向量；然后用一个最小训练实验证明：**同组共现的 token 向量会自动聚拢，跨组的保持距离**。完成后你会理解 LLM 输入层的唯一工作——查表取行。

---

## 为什么需要 Embedding？

上一章我们把文字变成了整数序列。但神经网络的计算单元（矩阵乘法、加法、激活函数）只能处理连续数值，而 token 本质上是整数 ID：`'A'` 是 65，`'B'` 是 66，`'你'` 可能是 15267。
...
Embedding 解决的是"整数变向量"的问题。核心操作就是查表取行——`weight[token_id]`，等价于 `nn.Embedding`。初始化时权重随机，任意两个 token 的向量没有语义关系；训练后，通过 next-token prediction 的梯度反传，频繁共现的 token 向量被拉近，形成有意义的几何结构。one-hot 编码做不到这一点，因为所有词对的距离恒为 √2，无法编码语义差异。

---

## 下一步

现在每个 token 都有了一个向量表示，但这些向量是各自独立的——"我吃苹果"里的"苹果"不知道前面有个"吃"。怎样让模型学会"哪些 token 之间需要相互关注"？

→ **Attention：词语间的关联计算**——注意力机制让每个 token 能看到序列中的所有其他 token，从而捕捉长距离依赖。
