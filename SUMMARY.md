...
设太大 → 每个请求等待的其他请求变多，平均延迟上升
         且 KV Cache 可能不够，触发频繁 Preemption（见 step05b）
...
控制每步最多处理的 prefill token 数（step05a）。
...
GPU 显存利用率
  KV Cache 占用：应尽量高（步骤 step06 的目标是 ~96%）
...
Preemption 次数
  频繁 Preemption（step05b）说明 KV Cache 不够用
...
| 优化手段 | 对应步骤 | 主要收益 | 代价 |
|---------|---------|---------|------|
| KV Cache | step03a | Decode 速度提升 10-100× | 显存占用增加 |
| Continuous Batching | step04 | 吞吐量提升 2-5× | 实现复杂度 |
| Chunked Prefill | step05a | TPOT 稳定，TTFT 改善 | 长 prompt TTFT 微增 |
