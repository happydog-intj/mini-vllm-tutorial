...
✅ step09_real_model 通过（跳过推理测试，未找到模型）
```
...
本节的引擎每次只处理一个请求，注意力计算也是标准实现。下一步（step10）将引入 **FlashAttention**：通过分块计算消除标准注意力的内存带宽瓶颈，同时支持变长序列（varlen）接口，为后续 Continuous Batching 做好基础。
