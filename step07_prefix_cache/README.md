...
这个思路和 [step03a 的 KV Cache](../step03a_kvcache_single/README.md) 一脉相承：

```
step03a KV Cache（请求内复用）：
  同一请求内，历史 token 的 K/V 只算一次，后续 decode 步骤直接读缓存
  复用边界：单个请求的历史 token

step07 Prefix Cache（跨请求复用）：
  不同请求共享相同前缀时，前缀的 K/V 只算一次，后续请求直接复用
  复用边界：多个请求之间的公共前缀
```

两者的底层逻辑完全相同——`K_i = x_i · W_K`，只要 token 序列和位置相同，K/V 就相同，可以缓存。区别只在于**复用的粒度**：KV Cache 是"同一请求跨时间步复用"，Prefix Cache 是"不同请求跨请求边界复用"。
