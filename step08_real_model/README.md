...
本节的引擎有一个明显的限制：**每次只处理一个请求**。

下一步要解决的问题是 **连续批处理（Continuous Batching）**：如何让多个用户的请求同时在一块 GPU 上运行，让 GPU 的矩阵乘法计算单元始终保持忙碌，而不是等待某一个请求生成完毕再处理下一个。

这需要把多个长度不同的序列拼在一起处理，引入 PagedAttention 管理 KV Cache 的动态分配，以及调度器决定哪些请求参与当前的 decode 步骤。
