...
```
T=0.1 时，logits=[8.7, 0.1, -2.3] → scaled=[87, 1, -23]
softmax 后概率几乎全压在 token 0
```

**T = 1**：不改变原始分布。

**T > 1（高温）**：logits 被压缩，差距缩小，分布趋向均匀：

```
T=2.0 时，logits=[8.7, 0.1, -2.3] → scaled=[4.35, 0.05, -1.15]
softmax 后各 token 概率差距缩小，"意外"token 更容易被选到
```
...
step03 引入 KV Cache，把历史 token 的 K/V 矩阵缓存起来，让每步推理只计算最新 token 的 attention，把计算量从 O(n²) 降到 O(n)——这是 LLM 推理加速的核心机制。
