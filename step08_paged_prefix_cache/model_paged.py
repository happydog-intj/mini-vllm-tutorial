"""
step08: TinyTransformerPaged — 用 block_table + kv_pool 替代 past_key_values

核心变化（对比 step03a 的 TinyTransformerWithKVCache）：
...
def gather_kv_from_blocks(
    ...
    for block_id in block_table:
        ...
        chunks.append(pool[block_id, :slots])  # [slots, num_heads, d_head]
        remaining -= slots
    return torch.cat(chunks, dim=0)  # [seq_len, num_heads, d_head]
...
class PagedMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        # 把当前 token 的 K/V 写入 kv_pool 对应的物理槽位
