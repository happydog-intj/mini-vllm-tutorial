...
import time
import torch
from engine import NoPrefixCacheEngine, PagedPrefixCacheEngine
...
def main():
    ...
    print("耗时对比")
    print("=" * 60)
    no_cache = NoPrefixCacheEngine()
    t0 = time.perf_counter()
    no_cache.generate_batch(requests)
    t_no = time.perf_counter() - t0
    print(f"  无前缀缓存:          {t_no*1000:.1f} ms")
    print(f"  有前缀缓存（第一轮）: {t_cold*1000:.1f} ms")
