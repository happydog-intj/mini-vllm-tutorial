...
> "苹果"应该关注"吃"还是"手机"？注意力机制让每个 token 自己决定从序列的哪些位置收集信息。

## 这一章做什么？

从零实现 Scaled Dot-Product Attention 和 Multi-Head Attention。完成后你会理解 Q/K/V 三个矩阵各自的角色、为什么要除以 √d、因果 mask 如何防止模型"偷看未来"。

---

## 为什么需要注意力机制？

上一章每个 token 获得了自己的向量表示，但这些向量是孤立的——"我吃苹果"里的"苹果"不知道前面有个"吃"。模型需要一种机制让 token 之间交换信息。
...
注意力机制解决的是"token 之间如何交换信息"的问题。Q/K/V 三个矩阵各司其职：Q 表示"我需要什么"，K 表示"我能提供什么"，V 是实际内容。点积 Q·K 度量匹配程度，除以 √d 防止 softmax 退化为 one-hot，因果 mask 遮住未来位置防止作弊。多头注意力把维度切成若干份独立计算，让不同头学习不同类型的依赖关系。

---

## 下一步

注意力只是 Transformer 的三大组件之一。单靠注意力，模型只能做"加权混合"——把已有信息搅在一起，但不能产生新的非线性特征。谁来负责"思考"？

→ **Transformer Decoder 层：组装完整计算单元**——加入 MLP（SwiGLU）做非线性变换、RMSNorm 做归一化，三个组件拼成一个完整的 Decoder 层。
