...
_step03a_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'step03a_kvcache_single')
)
...
# ─────────────────────────────────────────────────────────────────────
# 支持 batch 维度的模型封装
# step03a 的 TinyTransformerWithKVCache 只支持 [seq_len]，
...
class BatchKVCacheEngine:
    ...
    Decode 阶段：
      - 所有请求同步推进，等最长请求完成
      - 已完成的请求继续占用槽位（Static Batching 的核心缺陷，step04 解决）
    """
    ...
    def generate_batch(
        ...
        # （这是 Static Batching 的第二个缺陷，step04 的 Continuous Batching 解决）
