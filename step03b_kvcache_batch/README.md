...
GPU 计算单元利用示意（每行 = 1个时间片）:
              ┌──────────────────────────────────────┐
  时间片 1:   │ 同时算全部4个请求 ████████████████████ │ ← 几乎满载
              └──────────────────────────────────────┘
  计算单元利用率: ~85%，且只有 1 次 GPU 调度开销。
```
...
大矩阵 [4×30, 1024] @ [1024, 1024]（batch 合并后）：
  约 2.5 亿次乘加运算
  可以占用 GPU 约 85% 的计算单元
  → 完成同样 4 倍的总工作量，时间只需约 1.3 倍（而不是 4 倍）
```

### 关键数字感受（Qwen3-0.6B，hidden=1024，A100 实测）

| 方式 | 矩阵乘法的规模 | 耗时 |
|------|--------------|------|
| 串行，4 次分开算 | 4 × [30, 1024] @ [1024, 1024] | ≈ 3.2ms |
| Batch，1 次合并算 | 1 × [4×30, 1024] @ [1024, 1024] | ≈ 0.9ms（**3.5×**）|
