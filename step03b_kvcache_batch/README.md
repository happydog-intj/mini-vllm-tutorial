...
## 背景：step03a 解决了单请求，但生产环境需要同时服务多个用户

step03a 引入了 KV Cache，让单个请求的推理从 O(n²) 降到 O(n)。但实际推理服务面对的是**多个并发用户**：
...
Continuous Batching（step04 引入）：
  step 1~10：[请求A, 请求B, 请求C, 请求D]
  step 11：  请求B 完成 → 立刻释放槽位，新请求E 填入
  step 11+：[请求A, 请求E, 请求C, 请求D]  ← 槽位持续被占满
```

效果：GPU 的 decode 槽位永远被有效请求占满，不存在空转。这是 vLLM 高吞吐的核心机制之一，将在 step04 详细实现。
