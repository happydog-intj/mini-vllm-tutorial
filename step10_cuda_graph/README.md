...
到这一步，我们已经优化了 decode 阶段的**调度延迟**。但在生产系统中，还有一个重要问题没有解决：**多个用户请求如何高效共享 GPU 上的 KV cache 内存**？

当前的静态 KV cache 分配方式（每个请求预分配最大长度的内存）会导致严重的内存浪费——大量内存被预留但未使用，限制了系统能同时服务的请求数。

下一步将介绍 **PagedAttention**：借鉴操作系统虚拟内存的分页思想，把 KV cache 切分成固定大小的"页"，按需分配，大幅提升内存利用率，支持更大的并发请求数。
