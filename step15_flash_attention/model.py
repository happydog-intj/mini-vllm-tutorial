"""
step15: FlashAttention — 用 F.scaled_dot_product_attention 替代手写 bmm attention

在 step14_7 基础上的改动：
  - PagedMultiHeadAttention.forward：attention 计算改用 F.scaled_dot_product_attention
    - CPU / 无 flash_attn：PyTorch 自动选择 math / memory-efficient kernel
    - GPU + flash_attn 安装：自动使用 FlashAttention kernel，O(n) HBM 访问
  - forward_batched：同样替换，消除 bmm + 手写 causal mask
  - is_causal 参数替代手写 causal mask 构造逻辑
...
class PagedMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        # 对所有头批量做 Attention — F.scaled_dot_product_attention
        # GPU + flash_attn 安装时自动使用 FlashAttention kernel（O(n) HBM）
        # CPU / fallback 时使用 PyTorch math attention
        # [seq_len, num_heads, d_head] → [1, num_heads, seq_len, d_head]（SDPA 要求 4D）
        Q_t = Q.transpose(0, 1).unsqueeze(0)      # [1, num_heads, seq_len, d_head]
        K_t = K_full.transpose(0, 1).unsqueeze(0) # [1, num_heads, total_len, d_head]
        V_t = V_full.transpose(0, 1).unsqueeze(0) # [1, num_heads, total_len, d_head]

        # decode 时 seq_len=1，不需要因果 mask（只有一个 query，无未来位置）
        # prefill 时 start_pos=0，is_causal=True 生成标准下三角 mask
        # prefill + start_pos>0（prefix cache 命中后续 chunk）：需要手动构造 attn_mask
        if seq_len == 1 or start_pos == 0:
            ...
            # prefix cache 命中后的 chunk prefill：start_pos > 0，需要显式 mask
            q_idx = torch.arange(seq_len, device=x.device).unsqueeze(1)
            k_idx = torch.arange(total_len, device=x.device).unsqueeze(0)
            attn_mask = ~(k_idx > (start_pos + q_idx))  # True = 允许 attend
            out = torch.nn.functional.scaled_dot_product_attention(
                Q_t, K_t, V_t, attn_mask=attn_mask.unsqueeze(0).unsqueeze(0)
            )

        out = out.squeeze(0).transpose(0, 1).reshape(seq_len, -1)  # [seq_len, d_model]
        return self.W_o(out)
    ...
    def forward_batched(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            for i in range(num_seqs):
                ...
                # F.scaled_dot_product_attention（替代手写 bmm + causal mask）
                # 需要 4D：[batch=1, num_heads, seq_len, d_head]
                ...
                if seq_len == 1 or start_pos == 0:
                    ...
                    # prefix cache 命中后的 chunk prefill：start_pos > 0，显式 mask
                ...
                out_i = out_i.squeeze(0).transpose(0, 1).reshape(seq_len, -1)
