"""
step14_5: Paged Prefix Cache + 增量 Hash

在 step14_4 基础上的改动：
  - _lookup_prefix_cache 接收 Sequence，利用 seq._block_hashes 避免重算 hash
  - _save_prefix_cache 直接更新 seq._block_hashes / seq._prev_hash
  - 冷请求首次 lookup 时一次性预计算，后续增量维护
"""
...
class PagedPrefixCacheEngine:
    ...
    def _free_seq(self, seq: Sequence):
        ...
        cached_ids = seq.block_table[:len(seq.block_table) - len(seq._new_blocks or [])]
